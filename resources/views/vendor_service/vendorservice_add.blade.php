<?php
if($mode=="parts") {
        $vcat='vendorservice/parts';
        $title="Parts ";
        $back_url='vendorservice_add/parts';
    }
?>
@extends('layouts.app', ['page' => __('Parts'), 'pageSlug' => 'vendorservice/parts'])
<style>
select > option {
    color: black;
}
            ...
            <div class="card">
                ...
                <div class="col-12">
                    <nav aria-label="breadcrumb" role="navigation">
                        ...
                        <li class="breadcrumb-item "><a href="{{ $back_url }}">{{ $title }}</a></li>
                ...
                <form method="post" action="" id="servicecreate" autocomplete="off">
                    ...
                    <div class="alert alert-success" style="display: none">
                        ...
                        <span>
                          <b> {{ $title }} Successfully Added </b></span>
                      </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control{{ $errors->has('servicetype_list') ? ' is-invalid' : '' }} selectpicker " data-style="select-with-transition" title="Single Select" data-size="7" placeholder="{{ __('Service Type') }}" name="servicetype_list" id="servicetype_list" value="{{ old('servicetype_list') }}">
                            ...
                            @foreach($servicelist_vendor as $list)
                                                <option value="{{$list->id}}" tableconnected="{{ $list->table_connected }}" readonly="true" @if(old('servicetype_list') == $list->id) selected @endif >{{$list->name}}</option>
                                            @endforeach
                        </select>
       ...
       $('select[name="vendor_name"]').on('change', function() {
           var vendor_id = $(this).val();
            ...
            $(document).on('click', '#submitForm', function(){
                ...
                var ref=$('#hidden_mode').val();
                $.ajax({
                    url: '../vendorservice_insert',
                    method:'post',
                    data:formData,
                    success:function(data) {
                        ...
                        if(data.errors) {
                            ...
                            window.location="../vendorservice/"+ref;
