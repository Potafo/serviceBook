...
class VendorServiceController extends Controller
{
    ...
    public function services_view($mode)
    {
        $category='';
        if($mode=="parts"){
        ...
        ->leftJoin('service_pricedetails','service_pricedetails.service_id','=','service.id')
        ->where( 'service.type', '=', $category);
        $services=array();
        if(Session::get('logged_user_type') =='3')
        {
            $vendor_id=Session::get('logged_vendor_id');
            $services= $rows1->where('service.vendor_id','=',$vendor_id)
                ->select('service.*','products.name as pdtname','service.name as sername','service_type.name as sname','service_pricedetails.actual_price')
                ->paginate(Session::get('paginate'));
        }
        else if(Session::get('logged_user_type') =='1')
        {
            $services=$rows1->join('vendor', 'vendor.id', '=', 'service.vendor_id')
                ->select('service.*','products.name as pdtname','service.name as sername','service_type.name as sname','vendor.name as vname','products.name as pdtname','service_pricedetails.actual_price')
                ->paginate(Session::get('paginate'));
        }
       return view('vendor_service.vendorservice',compact('mode','services'));

    }
    public function services_add($mode)
    {
        $category='';
        if($mode=="parts"){
           $category='Parts';
        ...
        ->join('vendor_servicetype', 'service_type.id', '=', 'vendor_servicetype.service_type')
         ...
         ->where('service_type.name','=',$category)
         ->paginate(Session::get('paginate'));
         //dd(DB::getQueryLog());
        return view('vendor_service.vendorservice_add', compact('mode','products','servicelist_vendor'));
    }
    ...
    public function insert(Request $request)
    {
        ...
        if($validator->fails()) {
           ...
           $this->add_service($request);
            return Redirect('vendorservice/'.$request['hidden_mode'])->with('status', 'Services Successfully Added!');
